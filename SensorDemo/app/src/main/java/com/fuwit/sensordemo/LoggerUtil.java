...
public class LoggerUtil {
    ...
    public static void log(String tag,String message, int logLevel) {
        ...
        if(logLevel <= 1)
        {
            Log.e(String.format("chris_debug:[%s] ", tag), message);
        }
        else
        {
            Log.d(String.format("chris_debug:[%s] ", tag), message);
        }
...
//        }
    ...
    public static void error(String tag,String message,Exception ex) {
        ...
        Log.e(tag, String.format("%s, error: %s", message, ex));
        log(tag, String.format("%s, error: %s", message, ex), LOG_LEVEL_ERROR);
    }
